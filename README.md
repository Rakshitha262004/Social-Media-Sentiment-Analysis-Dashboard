...
### Option C — Import Xquik tweet exports

Export tweet search results from Xquik as JSON, JSONL, or CSV, then convert them
into the dashboard dataset shape:

```bash
python src/xquik_import.py xquik-tweets.json --output data/social_media_posts.csv --brand "Launch Watch"
python src/preprocess.py
python src/train_model.py
streamlit run app/dashboard.py
```

Rows without a `sentiment` field default to `neutral`; pass
`--sentiment positive` or `--sentiment negative` when importing a pre-filtered
campaign export.
...
Xquik tweet exports can also be converted with `python src/xquik_import.py` before preprocessing.
